...
**Hovedfanene er en `tablist`.** `Lister | Notater` følger WAI-ARIA-mønsteret:
`role="tablist"`/`role="tab"` med `aria-selected`, `aria-controls` mot hver sin
`role="tabpanel"`, venstre/høyre piltast bytter fane, og bare den aktive fanen
er i tabbrekkefølgen (`tabindex="-1"` på den andre) — så Tab går VIDERE til
innholdet i stedet for mellom to knapper.

**Notatene har de samme snarveiene som alt annet.** `Alt`+pil flytter et
notatkort, en notatmappe eller et notatprosjekt ett hakk (`canReorderObj` svarer
...
**I editoren** flytter Tab som vanlig mellom tilbakeknapp, verktøylinje, tittel
og skrivefeltet. Verktøyknappene tar ALDRI markeringen fra dokumentet
